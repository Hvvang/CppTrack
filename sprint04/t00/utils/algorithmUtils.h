...
void RemoveAll(Collection& c, const T& value) {
    auto iter = std::remove(begin(c), end(c), value);
    Collection new_front(std::distance(c.begin(), iter));
    Collection new_back(std::distance(iter, c.end()));

    move(c.begin(), iter, new_front.begin());
    move(iter, c.end(), new_back.begin());
    c = new_front;
}

template <class Collection, class Pred>
void RemoveAllIf(Collection& c, Pred&& predicate) {
    auto iter = std::remove_if(begin(c), end(c), predicate);
    Collection new_front(std::distance(c.begin(), iter));
    Collection new_back(std::distance(iter, c.end()));
...
void Sort(Collection& c, Comp&& comparator) {
    for (auto i = c.begin() + 1; i != c.end(); i++) {
        auto key = *i;
        auto j = i - 1;
        while (j != c.begin() && comparator(*j, key)) {
            *(j + 1) = *j;
            j--;
        }
        *(j + 1) = key;
...
void Unique(Collection& c) {
    ...
    move(c.begin(), it, uniqueElements.begin());
    move(it, c.end(), toDelete.begin());
    c = move(uniqueElements);
}
